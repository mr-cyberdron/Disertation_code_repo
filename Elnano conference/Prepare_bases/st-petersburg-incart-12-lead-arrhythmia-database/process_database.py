...
db_source = 'E:/Bases/st-petersburg-incart-12-lead-arrhythmia-database-1.0.0/files/'
storing_path = 'E:/Bases/0Prepared_bases/st-petersburg-incart-12-lead-arrhythmia-database_prep/'
...
for file in files:
    ...
    if file in files_with_pecularities:
        for cond in peculiarities[file]:
            if cond == 'skip_rec':
                save_flag = False
            if cond == 'skip_lead_11':
                signals, re_format, units, adc_gain, baseline, lead_names = remove_leads(
                    signals,re_format,units,adc_gain,baseline,lead_names,lead_idx=11)
            if cond == 'skip_lead_8':
                signals, re_format, units, adc_gain, baseline, lead_names = remove_leads(
                    signals,re_format,units,adc_gain,baseline,lead_names,lead_idx=8)
            if cond == 'skip_lead_0':
                signals, re_format, units, adc_gain, baseline, lead_names = remove_leads(
                    signals,re_format,units,adc_gain,baseline,lead_names,lead_idx=0)
            if cond == 'skip_lead_6':
                signals, re_format, units, adc_gain, baseline, lead_names = remove_leads(
                    signals,re_format,units,adc_gain,baseline,lead_names,lead_idx=6)
            if cond == 'skip_lead_2':
                signals, re_format, units, adc_gain, baseline, lead_names = remove_leads(
            ...
            if cond == 'skip_lead_9':
                signals, re_format, units, adc_gain, baseline, lead_names = remove_leads(
                    signals,re_format,units,adc_gain,baseline,lead_names,lead_idx=9)
