...
# Custom polynomial activation with trainable parameters
class CustomPolynomial(nn.Module):
    def __init__(self, degree=3):
        super().__init__()
        self.coeffs = nn.Parameter(torch.randn(degree + 1))  # degree + 1 coefficients

    def forward(self, x):
        # Evaluate the polynomial: c_0 + c_1*x + c_2*x^2 + ... + c_n*x^n
        result = 0
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.act1 = CustomPolynomial(degree=3)

        # Batch normalization and Dropout added previously
        self.layer2 = nn.Linear(512, 256)
        self.bn1 = nn.BatchNorm1d(256)
        self.act2 = CustomPolynomial(degree=3)
...
# Plot the resulting polynomial functions
x_vals = torch.linspace(-1, 1, 100)  # Limit range to avoid very large values
with torch.no_grad():
    y_vals_1 = model.act1(x_vals)
    y_vals_2 = model.act2(x_vals)

plt.figure(figsize=(12, 6))
plt.plot(x_vals.numpy(), y_vals_1.numpy(), label=f'Polynomial 1 (degree={len(model.act1.coeffs) - 1})')
plt.plot(x_vals.numpy(), y_vals_2.numpy(), label=f'Polynomial 2 (degree={len(model.act2.coeffs) - 1})')
plt.title("Trained Polynomial Activation Functions")
plt.xlabel("x")
plt.ylabel("Polynomial(x)")
plt.legend()
plt.grid(True)
plt.show()
