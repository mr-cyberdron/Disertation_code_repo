...
X1 =np.load('./trainDAta/SiamseNet/LAP/X_lap_train.npy')
X2 =np.load('./trainDAta/SiamseNet/LAP/X1_lap_train.npy')
Y =np.load('./trainDAta/SiamseNet/LAP/Y_lap_train.npy')
...
X1 =np.load('./trainDAta/SiamseNet/LAP/X_lap_test.npy')
X2 =np.load('./trainDAta/SiamseNet/LAP/X1_lap_test.npy')
Y =np.load('./trainDAta/SiamseNet/LAP/Y_lap_test.npy')
...
dataloader = DataLoader(dataset,batch_size=len(X1_tensor), shuffle=True)

for input1, input2, label in dataloader:
    input1, input2, label = input1.to(device), input2.to(device), label.to(device)

    similarity_score = model(input1, input2).squeeze()
    compute_metrics(similarity_score, label)
