...
# Custom Swish activation with trainable parameter
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.act1 = CustomSwish(initial_beta=1.0)

        # Batch normalization and Dropout added previously
        self.layer2 = nn.Linear(512, 256)
        self.bn1 = nn.BatchNorm1d(256)
        self.act2 = CustomSwish(initial_beta=1.0)
...
# Plot the resulting Swish functions
...
plt.plot(x_vals.numpy(), y_vals_1.numpy(), label=f'Swish 1 (beta={model.act1.beta.item():.4f})')
plt.plot(x_vals.numpy(), y_vals_2.numpy(), label=f'Swish 2 (beta={model.act2.beta.item():.4f})')
plt.title("Trained Swish Activation Functions")
plt.xlabel("x")
plt.ylabel("Swish(x)")
plt.legend()
plt.grid(True)
